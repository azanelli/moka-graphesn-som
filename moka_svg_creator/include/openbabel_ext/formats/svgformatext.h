#ifndef OB_SVGFORMATEXT_H
#define OB_SVGFORMATEXT_H
...
#include <openbabel_ext/depict/depictext.h>
#include <openbabel_ext/depict/svgpainter.h>
 ...
 * Class SVGFormatExt
 *
 * SVGFormat class, plus option "L" to print custom atom labels. You can put
 * custom label in the aotom data as an object of type OBPairData in the
 * attributes:
 *   - <CustomLabelText>: the text to print out.
 *   - <CustomLabelColor>: text color as three RGB values comma separated
 *       (optional, green is the default).
 */
class SVGFormatExt : public OBFormat
{
  public:
     ...
     * If the first object is OBText, the part of it before each insertion
     * point (if it exists) is output before every molecule. This allows
     * molecule structures to be displayed in a template. The x option to omit
     * the XML header is set.
     */
    bool WriteChemObject(OBConversion* pConv)
    {
      ...
      if (pConv->GetOutputIndex() <= 1)
      {
        ...
        // to show WriteMolecule that this function has been called
        pConv->AddOption("svgwritechemobject");

        // alternative for babel because -xc cannot take a parameter, because
        // some other format uses it similarly for -xr -xp
      ...
      bool nomore = _nmax && (_objects.size() == (unsigned int)_nmax);
      ...
      // OBConversion decrements OutputIndex when returns false because it
      // thinks it is an error. So we compensate.
      if (!ret || nomore)
    ...
    bool WriteMolecule(OBBase* pOb, OBConversion* pConv)
    {
      ...
      if ((pConv->IsOption("y") || !pmol->Has2D(true)) && !pConv->IsOption("n"))
      {
        ...
        if (!pOp)
        {
          obErrorLog.ThrowError
              ("SVGFormat", "gen2D not found", obError, onceOnly);
          return false;
        }
        if (!pOp->Do(pmol))
        {
      ...
      // allows 3D coordinates (if passed by -xn above)
      if(!pmol->Has2D() && pmol->NumAtoms() > 1)
      {
        std::string mes("Molecule ");
        mes += pmol->GetTitle();
      ...
      std::string background, bondcolor;
      ...
      if (pConv->GetOutputIndex() == 1 || fixedpx)
      {
        ...
        if (hasTable)
        {
          ...
          // and no width or height - it uses the whole of its containing
          // element. Inner svg with width, height, x, y of table cell,
          ...
          // fixed size image
          if (fixedpx)
            ofs << "x=\"0\" y=\"0\" width=\"" << fixedpx
                << "px\" height=\"" << fixedpx <<"px\" ";
          else
            ofs << "x=\"0\" y=\"0\" width=\"100%\" height=\"100%\" ";

          ofs << "viewBox=\"0 0 " << vbwidth << ' ' << vbheight << "\">\n";

          ofs << "<title>Prediction Results - Moka</title>\n";
          // Draw the background unless transparent
          if (!transparent)
            ofs << "<rect x=\"0\" y=\"0\" width=\"" << vbwidth
                << "\" height=\"" << vbheight << "\" fill=\"" << background
                << "\"/>\n";
        } // if (hasTable)
      ...
      if (hasTable)
      {
        ...
        if (htxt)
        {
          ...
          tokenize(vec, htxt);
          std::string highlight(vec.size() > 1 ? vec[1] : "#f4f0ff");
        ...
        if (!pConv->IsOption("d"))
        {
          //ofs << "<text text-anchor=\"middle\" font-size=\""
          //    << 0.06 * cellsize << "\"" << " fill =\"" << bondcolor
          //    << "\" font-family=\"sans-serif\"\n" << "x=\""
          //    << innerX + cellsize * 0.5 << "\" y=\""
          ...
          ofs << "<text text-anchor=\"middle\" font-size=\""
              << 0.05 * cellsize << "\"" << " fill =\"" << bondcolor
              << "\" font-family=\"sans-serif\"\n" << "x=\""
              << innerX + cellsize * 0.5 << "\" y=\""
              << (innerY + cellsize - 2.0 / nr) + text_offset << "\" >"
              << pmol->GetData("InfoTox")->GetValue() << "</text>\n";

          ofs << "<text text-anchor=\"middle\" font-size=\""
              << 0.03 * cellsize << "\"" << " fill =\"" << bondcolor
              << "\" font-family=\"sans-serif\"\n" << "x=\""
              << innerX + cellsize * 0.5 << "\" y=\""
              << (innerY + cellsize - 2.0 / nr) +
                 (text_offset += 0.04 * cellsize) << "\" >";

          ofs << pmol->GetData("InfoBias")->GetValue() <<" | "
              << pmol->GetData("InfoPValue")->GetValue();

          if (pmol->HasData("InfoTValue"))
            ofs << " | " << pmol->GetData("InfoTValue")->GetValue();

          ofs << "</text>\n";

          if (pmol->HasData("InfoID"))
            ofs << "<text text-anchor=\"middle\" font-size=\""
                << 0.03 * cellsize << "\"" << " fill =\"" << bondcolor
                << "\" font-family=\"sans-serif\"\n" << "x=\""
                << innerX + cellsize * 0.5 << "\" y=\""
                << (innerY + cellsize - 2.0 / nr) +
                   (text_offset += 0.04 * cellsize) << "\" >"
                << "Molecule ID = "
                << pmol->GetData("InfoID")->GetValue() << "</text>\n";
        ...
        OBDepictExt depictor(&painter);

        if(!pConv->IsOption("C"))
          depictor.SetOption(OBDepictExt::drawTermC); // on by default
        if(pConv->IsOption("a"))
          depictor.SetOption(OBDepictExt::drawAllC);
        ...
        if (pConv->IsOption("u"))
          depictor.SetOption(OBDepictExt::bwAtoms);
        if (!pConv->IsOption("U"))
          depictor.SetOption(OBDepictExt::internalColor);
        if (pConv->IsOption("s"))
          depictor.SetOption(OBDepictExt::asymmetricDoubleBond);

        depictor.DrawMolecule(pmol);

        // Draw atom indices if requested
        if (pConv->IsOption("i"))
          depictor.AddAtomLabels(OBDepictExt::AtomIndex);

        // Draw custom atom labels
        if (pConv->IsOption("L"))
          depictor.AddAtomLabels(OBDepictExt::AtomCustomLabel);
      ...
      else // no table
      {
        ...
        if (pConv->IsOption("W"))
          depictor.SetOption(OBDepictExt::noWedgeHashGen);
        if (!pConv->IsOption("C"))
          depictor.SetOption(OBDepictExt::drawTermC); // on by default
        if (pConv->IsOption("a"))
          depictor.SetOption(OBDepictExt::drawAllC);
        ...
        // ofs << "<title>" << pmol->GetTitle() << " - SVG Depiction</title>\n";
        ofs << "<title>" << pmol->GetTitle() << " - Moka</title>\n";
      ...
      if (hasTable && pConv->IsLast())
      {
        ...
        if(_nrows && _ncols && pConv->IsOption("l"))
        {
          for (int i = 1; i < _nrows; ++i)
            ofs << " <line  stroke=\"gray\" stroke-width=\"0.1\" x1=\"0\""
                << " x2=\"100\" y1=\""  << i*cellsize << "\" y2=\""
                << i * cellsize << "\"/>\n";
          for (int i = 1; i < _ncols; ++i)
            ofs << " <line  stroke=\"gray\" stroke-width=\"0.1\" y1=\"0\""
                << " y2=\"100\" x1=\""  << i * cellsize << "\" x2=\""
                << i * cellsize << "\"/>\n";
        }
      ...
      return !fixedpx; // false with fixed size image because only 1 mol
    } // method WriteMolecule
  ...
  private:
     ...
     */
    bool EmbedScript(std::ostream& ofs)
    {
      std::ifstream ifs;
  ...
  private:
    ...
    std::vector<OBBase*> _objects;
    OBText* _ptext;
    std::string::size_type _textpos;

}; // class SVGFormatExt

// SVGFormatExt theSVGFormatExt;

} // namespace OpenBabel

#endif // OB_SVGFORMATEXT_H
